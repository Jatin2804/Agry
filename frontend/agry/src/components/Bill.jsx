...
const Bill = () => {
  const { billData, setBillTotal, setBillData } = useAppContext(); 
  ...
  useEffect(() => {
    const total = billData.reduce((acc, item) => {
      return acc + item.sp * (quantities[item.name] || 1); 
    }, 0);
    setBillTotal(total); 
  }, [billData, quantities, setBillTotal]);
  ...
  const handleDelete = (product) => {
    setBillData((prevData) => prevData.filter((item) => item.name !== product.name)); 
  };
    ...
    <div style={{ padding: "20px" }}>
      ...
      <Box sx={{ marginTop: "20px", textAlign: "center", display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center",width:"30opx" }}>
        ...
        <Box sx={{ marginTop: "10px" }}>
          {billData.map((product) => (
            <Box key={product.name} sx={{ display: "flex", justifyContent: "space-between", marginBottom: "10px" }}>
              ...
              <Typography variant="body2">Rs {product.sp * (quantities[product.name] || 1)}</Typography>
            </Box>
          ))}
        </Box>
        ...
        <Typography variant="h5" style={{ fontWeight: "bold", marginTop: "20px" }}>
          Total: Rs {billData.reduce((acc, item) => acc + item.sp * (quantities[item.name] || 1), 0)}
        </Typography>
      </Box>
